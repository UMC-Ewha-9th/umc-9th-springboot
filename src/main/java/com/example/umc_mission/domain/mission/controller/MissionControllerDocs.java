...
public interface MissionControllerDocs {
    ...
    @Operation(
            summary = "내가 도전 중인 미션 조회 API (개발 중)",
            description = "내가 도전 중인 미션 목록을 모두 조회합니다. 페이지네이션으로 제공합니다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "실패")
    })
    ApiResponse<MemberMissionResDto.myMissionPreviewListDto> getMyMissions(Long memberId, Status status, Integer page);
    ...
    ApiResponse<MemberMissionResDto.myMissionPreviewListDto> completeMemberMission(MemberMissionReqDto.completeMemberMissionDto dto);
